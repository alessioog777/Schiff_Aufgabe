...
function addWaypoint(lat, lon, type) {
    ...
    let emoji = '';
    if (type === 'mob') emoji = ' ❌';       // Beispiel: ❌ für MOB
    else if (type === 'anchor') emoji = ' ⚓️'; // ⚓️ für Anker

    const li = document.createElement("li");
    li.innerHTML = `${waypointNumber}; ${timestamp.replace(",", ";")} — <a href="https://www.google.com/maps?q=${lat},${lon}" target="_blank">${lat.toFixed(6)}, ${lon.toFixed(6)}</a>${emoji}`;
    ...
    // WICHTIG: Popup mit Nummer und Koordinaten anzeigen
