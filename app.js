...
navigator.geolocation.watchPosition(
    (position) => {
        ...
        let headingCalculated = 0;
        if (previousPosition) {
            const d = calculateDistance(previousPosition.lat, previousPosition.lon, lat, lon);
            if (d > 0.001) { // nur berechnen, wenn mehr als 1 m Unterschied
                headingCalculated = calculateBearing(previousPosition.lat, previousPosition.lon, lat, lon);
            }
        }

        headingDisplay.textContent = headingCalculated.toFixed(0);
...
document.getElementById("generatePDF").addEventListener("click", () => {
    ...
    leafletImage(map, function (err, canvas) {
        ...
        [...document.querySelectorAll("#waypointList li")]
            .reverse()
            .forEach((li) => {
...
if (link) {
    const coords = link.textContent.split(",").map(s => parseFloat(s.trim()));
...
if (previousCoords && lat !== null && lon !== null) {
    distanceNM = calculateDistanceNM(previousCoords.lat, previousCoords.lon, lat, lon).toFixed(2);
    bearing = calculateBearing(previousCoords.lat, previousCoords.lon, lat, lon).toFixed(0);
}
...
if (lat !== null && lon !== null) {
    previousCoords = { lat, lon };
        ...
        doc.autoTable({
            startY: startY,
            head: [["Wegpunkt", "Datum/Uhrzeit", "Koordinaten", "Typ", "Kurs (°)", "Distanz (NM)"]],
            body: rows,
            theme: "grid",
            ...
            didDrawCell: (data) => {
                if (data.column.index === 3) {
                    ...
                    if (img) {
                        ...
                        doc.addImage(img, "PNG", data.cell.x + 2, data.cell.y + 2, 6, 6);
                    }
                }
            }
        });
...
function calculateDistanceNM(lat1, lon1, lat2, lon2) {
    ...
    return km * 0.539957; // km → nautische Meilen
}
